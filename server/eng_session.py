...
class EngSession(object):
    ...
    def analyze_frame(self,frame = None):
        ...
        try:
            ...
            if(response.status_code >= 200 and response.status_code < 500):
                if(response.status_code == 200):
                    print(f"Session {self.openface_id} - Result from OpenFaceApi in {response.elapsed} seconds. Resp size: {sys.getsizeof(response.content)} B")
                    ...
                    if(face_data.get("image") != None):
                        ...
                        if(len(self.face_data_list) >= self.sequence_size):
                            ...
                            if self.seq_checker == True:
                                self.seq_checker == False
                                # adding SEQ_SIZE frames to sequence size for the next sequence to send, up to evaluate all video
                                self.sequence_size += EngSession.STARTING_SEQ_SIZE
                                sleep(0.01)
                                self.seq_checker = True
                            ...
                            eng_datas_to_send = []
                            eng_datas_to_send.append(OF_const.HEADER_ARR) # header
                            eng_datas_to_send.extend(self.face_data_list) # sequence content
                            json_obj = {"face_sequence":eng_datas_to_send}
                            
                            eng_resp = r.post(url=definitions.ENGRECAPI_HOST+definitions.ENGRECAPI_ENGAGEMENT_ENDPOINT,
                                              json=json_obj)
                            
                            if(eng_resp.status_code == 200):
                                r_json_obj = eng_resp.json()
                                print(f"Session {self.openface_id} - Result from EngRecApi: {eng_resp.content} in {eng_resp.elapsed} seconds. Resp size: {sys.getsizeof(eng_resp.content)} B")
                                self.last_eng = r_json_obj.get("eng_level")
                                                                
                                #emit('last_eng_out',{"eng_level":self.last_eng,"seq_size":self.sequence_size}, namespace='/session')
                            elif(eng_resp.status_code == 400):
                                json_obj = response.json()
                                status = json_obj.get("status")
                                message = json_obj.get("message")
                                print(f"Session {self.openface_id} --- ERROR --- status code:{eng_resp.status_code} - Message: {message}")
                            elif(eng_resp.status_code > 400):
                                self.session_valid = False
                                self.message = f"Session {self.openface_id} --- ERROR --- status code:{eng_resp.status_code} - Message: Error with EngRecApi request"
                    ...
                    print(f"Session {self.openface_id} --- INFO --- status code:{response.status_code} - Message: Face Not Found")
                elif(response.status_code == 206 or response.status_code == 400):
                    # Error: Unauthorized or different Bad Request error 
                    json_obj = response.json()
                    status = json_obj.get("status")
                    message = json_obj.get("message")
                    print(f"Session {self.openface_id} --- ERROR --- status code:{response.status_code} - Message: {message}")
                ...
                print(f"Session {self.openface_id} --- ERROR --- status code:{response.status_code} - Message: {response.content}")                    
                raise RuntimeError("ERROR: Server failure.")
        except r.ConnectionError as e:
            # if one of the components is off or not responding
            self.session_valid = False
            self.message = 'Webservers are not responding, abort connection. Check OpenFaceApi or EngRecApi are still running.'
            print("ERROR: Webservers are not responding, abort connection. Check OpenFaceApi or EngRecApi are still running.\n",e.strerror)
        ...
        eng_lev = self.last_eng if self.last_eng == None else round(self.last_eng,3)
        ...
        image_data, _ = base64str2webp_base64str(base64str=image_data, image_type="jpg",option="-q 80",temp_path="./temp") # convert from base64 jpeg to webp to use whammy.js
    ...
    def format_openface_datas(frame, confidence, timestamp, gaze, head_pose, au_i, au_p):
        '''
    ...
    def format_eng_out(frame_number, timestamp, eng_data = None, eng_val = None):
        '''
        ...
        if(eng_data == None):
            ...
            eng_out[frame_key] = frame_number
